...
function PortalChoice({onSelect,onSearch,phone,setPhone,searching,message}) {
  return <div className="screen-app grid min-h-screen place-items-center bg-gradient-to-br from-slate-100 via-blue-50 to-slate-200 p-4"><div className="w-full max-w-5xl py-8"><div className="mb-8 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#17365d] text-xl font-bold text-white">บย</div><h1 className="mt-4 text-3xl font-bold text-slate-900">ระบบบันทึกการเยี่ยมบ้านนักเรียน</h1><p className="mt-2 text-slate-500">โรงเรียนหาดใหญ่รัฐประชาสรรค์</p></div><div className="grid gap-5 md:grid-cols-2"><button onClick={()=>onSelect('student')} className="card group p-8 text-left transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-lg"><span className="grid h-12 w-12 place-items-center rounded-xl bg-blue-100 text-2xl">🎓</span><h2 className="mt-5 text-2xl font-bold text-[#17365d]">ระบบนักเรียนกรอกข้อมูล</h2><p className="mt-2 text-slate-500">กรอกแบบเยี่ยมบ้านและแนบรูปภาพ ส่งข้อมูลให้คุณครู</p><span className="mt-6 inline-block font-semibold text-blue-700">เข้าสู่แบบฟอร์ม →</span></button><button onClick={()=>onSelect('teacher')} className="card group p-8 text-left transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-lg"><span className="grid h-12 w-12 place-items-center rounded-xl bg-amber-100 text-2xl">👩‍🏫</span><h2 className="mt-5 text-2xl font-bold text-[#17365d]">ระบบคุณครู</h2><p className="mt-2 text-slate-500">เข้าสู่ระบบเพื่อดูรายการ แก้ไข และพิมพ์แบบบันทึก</p><span className="mt-6 inline-block font-semibold text-blue-700">เข้าสู่ระบบคุณครู →</span></button></div><form onSubmit={onSearch} className="card mt-5 border-blue-200 bg-gradient-to-r from-white to-blue-50 p-6 md:p-7"><div className="grid gap-5 md:grid-cols-[1fr_1.2fr]"><div><h2 className="text-xl font-bold text-[#17365d]">🔎 ค้นหารายงานของนักเรียน</h2><p className="mt-2 text-sm text-slate-500">ใช้เบอร์มือถือผู้ปกครองที่บันทึกไว้ เพื่อแก้ไขหรือพิมพ์ฟอร์ม</p></div><div><div className="flex flex-col gap-3 sm:flex-row"><input className="field-input flex-1 text-lg" type="tel" inputMode="numeric" autoComplete="tel" pattern="[0-9]{9,10}" maxLength={10} value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="0812345678" aria-label="เบอร์มือถือผู้ปกครอง" required/><button disabled={searching} className="rounded-xl bg-blue-700 px-6 py-3 font-semibold text-white disabled:opacity-50">{searching?'กำลังค้นหา...':'ค้นหารายงาน'}</button></div><p className="mt-2 text-xs text-slate-400">กรอกเฉพาะตัวเลข 9–10 หลัก ไม่ต้องใส่ขีด</p>{message&&<p className="mt-2 text-sm font-semibold text-red-600">{message}</p>}</div></div></form></div></div>
}
...
function App() {
  ...
  const printStudentRecord = async record => {
    const editToken=readJson(STUDENT_RECORDS,[]).find(item=>item.recordId===record.recordId)?.editToken
    if (!editToken) return setMessage('ไม่พบสิทธิ์พิมพ์รายการนี้ กรุณาค้นหาและยืนยันรายการใหม่')
    setMessage('กำลังเตรียมข้อมูลและรูปภาพสำหรับพิมพ์...')
    try {
  ...
  const searchPreviousStudent = async event => {
    ...
    try {
      ...
      setPhoneSearch('')
      if (!mode) { setMode('student'); window.location.hash='student'; setView('studentRecords') }
  ...
  if (!mode) return <PortalChoice onSelect={selectMode} onSearch={searchPreviousStudent} phone={phoneSearch} setPhone={setPhoneSearch} searching={studentSearching} message={message}/>
  ...
  return <><div className="screen-app min-h-screen bg-slate-100">
    ...
    <main className="mx-auto max-w-7xl px-4 py-7">{message&&<div className="mb-5 flex justify-between rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900"><span>{message}</span><button onClick={()=>setMessage('')}>×</button></div>}
      ...
      {mode==='student'&&view==='studentRecords'&&<>
        ...
        <form onSubmit={searchPreviousStudent} className="card mb-5 border-blue-200 bg-gradient-to-br from-white to-blue-50 p-5 md:p-7"><h3 className="text-xl font-bold text-[#17365d]">ค้นหาฟอร์มของนักเรียน</h3><p className="mt-1 text-sm text-slate-500">กรอกเบอร์มือถือผู้ปกครองที่บันทึกไว้ เพื่อเข้าแก้ไขหรือพิมพ์ฟอร์มของตนเอง</p><div className="mt-5 flex max-w-2xl flex-col gap-3 sm:flex-row"><input className="field-input flex-1 text-lg" type="tel" inputMode="numeric" autoComplete="tel" pattern="[0-9]{9,10}" maxLength={10} value={phoneSearch} onChange={e=>setPhoneSearch(e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="เบอร์มือถือ เช่น 0812345678" required/><button disabled={studentSearching} className="rounded-xl bg-blue-700 px-6 py-3 font-semibold text-white disabled:opacity-50">{studentSearching?'กำลังค้นหา...':'ค้นหาฟอร์ม'}</button></div><p className="mt-3 text-xs text-slate-400">กรอกเฉพาะตัวเลข 9–10 หลัก ไม่ต้องใส่ขีด</p></form>
