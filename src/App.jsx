...
function PrintableForm({record}) {
  ...
  return <div className="print-document"><article className="a4-form">
    ...
    <div className="a4-lines">
      <div className="a4-line">1. ชื่อ - สกุลนักเรียน <Fill width="72mm">{r.studentName}</Fill> ชั้น <Fill width="17mm">{r.classLevel}</Fill> / <Fill width="15mm">{r.room}</Fill> ชื่อเล่น <Fill width="24mm">{r.nickname}</Fill></div>
      <div className="a4-line">2. ที่อยู่ปัจจุบัน&nbsp;&nbsp;ชื่อหมู่บ้าน <Fill width="58mm">{r.villageName}</Fill> เลขที่ <Fill width="18mm">{r.houseNo}</Fill> ซอย <Fill width="36mm">{r.soi}</Fill> ถนน <Fill width="31mm">{r.road}</Fill></div>
      ...
      <div className="a4-line">3. ชื่อ - สกุล ผู้ปกครองนักเรียน <Fill width="77mm">{r.guardianName}</Fill> อาชีพ <Fill width="62mm">{r.guardianJob}</Fill></div>
      ...
      <div className="a4-line">4. สถานภาพบิดา - มารดา <Check active={status==='อยู่ร่วมกัน'}/> อยู่ร่วมกัน <Check active={status==='แยกกันอยู่'}/> แยกกันอยู่ <Check active={status==='หย่าร้าง'}/> หย่าร้าง <Check active={status==='บิดาเสียชีวิต'}/> บิดาเสียชีวิต <Check active={status==='มารดาเสียชีวิต'}/> มารดาเสียชีวิต <Check active={status==='บิดามารดาเสียชีวิต'}/> บิดามารดาเสียชีวิต</div>
      <div className="a4-line">5. รายได้เฉลี่ยต่อคน (รวมรายได้ครัวเรือน หารด้วยจำนวนสมาชิกทั้งหมด) <Fill width="48mm">{r.incomePerPerson}</Fill> บาท / เดือน</div>
      <div className="a4-line">6. โรคประจำตัวนักเรียน <Check active={r.hasDisease==='ไม่มี'}/> ไม่มีโรคประจำตัว <Check active={r.hasDisease==='มี'}/> มีโรคประจำตัว คือ <Fill width="72mm">{r.diseaseDetail}</Fill></div>
      <div className="a4-line">7. ระยะทางระหว่างบ้านไปโรงเรียน (ไป/กลับ) <Fill width="18mm">{r.distanceKm}</Fill> กิโลเมตร <Fill width="18mm">{r.distanceMeters}</Fill> เมตร ใช้เวลาเดินทาง <Fill width="13mm">{r.travelHours}</Fill> ชม. <Fill width="13mm">{r.travelMinutes}</Fill> นาที</div>
      <div className="a4-line">8. สภาพบ้านที่อยู่อาศัย ดังนี้</div>
      ...
      <div className="a4-line">9. ภาระงานความรับผิดชอบของนักเรียนที่มีต่อครอบครัว</div>
      ...
      <div className="a4-line">10. กิจกรรมยามว่างหรืองานอดิเรก <Fill width="128mm">{r.hobbies}</Fill></div>
      <div className="a4-line">11. พฤติกรรมเสี่ยง <Check active={r.riskBehaviors?.includes('สารเสพติด')}/> พฤติกรรมการใช้สารเสพติด <Check active={r.riskBehaviors?.includes('ทางเพศ')}/> พฤติกรรมทางเพศ <Fill width="45mm">{r.riskDetail}</Fill></div>
